...
function PersonalFAQSection({ items, t }: { items: FAQItem[]; t: (key: string) => string }) {
  const [open, setOpen] = useState<number | null>(null);

  return (
    <section style={{ backgroundColor: "#FFFFFF", borderTop: "1px solid rgba(0,0,0,0.06)", padding: "80px 24px" }}>
      <style>{`.faq-pers-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 56px; }
        @media (max-width: 768px) { .faq-pers-grid { grid-template-columns: 1fr; gap: 40px; } }`}</style>
      <div style={{ maxWidth: "1120px", margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: "48px" }}>
          ...
          <h2 style={{ fontSize: "clamp(1.8rem, 3vw, 2.4rem)", fontWeight: 900, color: "#0B1F33", margin: 0, letterSpacing: "-0.025em" }}>
            {t("faq.personalHeading")}
          </h2>
        </div>
        <div className="faq-pers-grid">
          {/* Left: Personal FAQs */}
          <div>
            {items.map((item, i) => (
              <FAQRow key={i} item={item} open={open === i} onToggle={() => setOpen(open === i ? null : i)} />
            ))}
            <a href="/faq#personal" style={{ fontSize: "13px", fontWeight: 700, color: "#1B3A6B", textDecoration: "none", display: "inline-block", marginTop: "16px" }}>
              {t("faq.viewAllPersonal")} →
            </a>
          </div>
          ...
          <div style={{ backgroundColor: "#F8F9FB", border: "1px solid #E8EDF5", borderRadius: "20px", padding: "36px 32px", display: "flex", flexDirection: "column", alignItems: "flex-start", gap: "16px" }}>
            ...
            <p style={{ fontSize: "15px", color: "#64748B", margin: 0, lineHeight: 1.6 }}>{t("faq.stillSub")}</p>
            <a href="sms:5619468261" style={{ display: "inline-flex", alignItems: "center", gap: "8px", padding: "14px 28px", borderRadius: "10px", backgroundColor: "#1B3A6B", color: "#FFFFFF", fontSize: "15px", fontWeight: 800, textDecoration: "none" }}>
...
export default function FAQ({ mode }: { mode: Mode }) {
  ...
  return <PersonalFAQSection items={personalItems} t={t} />;
}
